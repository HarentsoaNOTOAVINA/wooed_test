...
onBeforeMount(()=>{
    store.dispatch('RentalModule/fetchReceivedOfferLease', {theProperty :3});

})
onMounted(()=>{
    offers.value = computed(() => store.getters["RentalModule/getReceivedOfferList"]).value;
...
watch(  ()=>store.getters["RentalModule/getReceivedOfferList"],
        (newVal)=>{
            offers.value = newVal;         
        }
)
    ...
    <transition mode="out-in" appear name="fade-component">
        <div class="list-container">
            <OfferCard 
                ...
                :key="key" 
                :id="key"
                :thePropertyId="offer.theProperty.id"
                :status="offer.status"
