<template>
    <div class="boilerRepairEmergency__container">
        <div class="boilerRepairEmergency__container-nav">
            <ArrowBack @click="$emit('prev')" />
            <Paragraphe>Réparations urgentes</Paragraphe>
        </div>
        <hr class="separator" />

        <div class="boilerRepairEmergency__listContainer">
            <div
                class="boilerRepairEmergency__content"
        ...
        <div class="boilerRepairEmergency__btn" v-if="listIdExperts.length > 0">
            <Button @on-click="$emit('component', CostEmergency)"
                >Suivant</Button
            >
        </div>
    </div>
...
<script setup lang="ts">
    ...
    import CostEmergency from './CostEmergency.vue';
    ...
    let boilerRepairEmergency = ref<any>({
        region: '',
        speciality: 'Boiler',
    });
    ...
    function getDetailStore() {
        ...
        if (data.value.length != 0) {
            boilerRepairEmergency.value.region = data.value[0].region;
        } else {
            try {
                store.dispatch(
                ...
                boilerRepairEmergency.value.region = data.value.data.region;
            } catch (error) {
...
<style scoped lang="scss">
    .boilerRepairEmergency {
        ...
        &__btn {
            @apply fixed bottom-0 right-[18px] w-[100px] h-[50px];
        }
    }
