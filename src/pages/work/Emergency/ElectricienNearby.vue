    ...
    <div class="Electricien__container">
        <div class="Electricien__content-title">
            <Paragraphe class="title"
                >Electriciens disponibles à proximité</Paragraphe
            >
            <Button
                type-button="border"
                v-if="mailSent"
...
<script setup lang="ts">
    ...
    async function getListElectric() {
        try {
            loading.value = true;
            await store.dispatch('WorksModule/setListElectricien');

            let { code, data } =
                store.getters['WorksModule/getListElectricien'];
