...
<script lang="ts" setup>
    ...
    import { typeDataRent } from './utils/type-selected';
    ...
    const dataAddProperty = shallowRef<Array<any>>(leaseHouseDataClassic);
    ...
    watch(
        ...
        (value) => {
            if (value.toString() !== propertyType.value.toString()) {
                dataAddProperty.value = typeDataRent(value, allPropertyType);
    ...
    async function handleFinish() {
        console.log("user", userDetail.value)
        saveForm();
        loadingFinish.value = true;
    ...
    onBeforeRouteLeave((to, _, next) => {
        ...
        if (unsavedChanges.value && data.locationType) {
            if (
                pathToRedirect.value === '/no-connection' ||
                pathToRedirect.value === '/error'
            ) {
                console.log('ity ny path to redirect', pathToRedirect.value);
                handleLeavePage();
            } else {
