<script lang="ts" setup>
    ...
    async function handleSubscribe(item : any){
        console.log("S'inscire\n", item);
        loading.value = true;
        await store.dispatch('CommunityModule/subscribeGroupPurchase', item);
        loading.value = false;
        ...
        <div class="houseWarming__container">
            <div class="houseWarming__item">
                <div class="houseWarming__valid">
                    ...
                    <span
                        >Categorie :
                        <strong> {{publication.purchase.category}} </strong>
                    </span>
                    <span
                        >Date de fin :
                        <strong>{{formatDateFr(publication.purchase.dateCloture)}}</strong>
                    </span>
                    <span
                        >Prix :
                        <strong>{{publication.purchase.price}} €</strong>
                    </span>
                    <Button v-if="!publication.purchase.isRegistered" @click="handleSubscribe(publication.id)">
                        <LoadingButton size="xs" v-if="loading" />
                        <Paragraphe v-else>Je serai présent</Paragraphe> 
                    </Button>
                    <div class="" v-else>
                        <Paragraphe>Vous êtes déjà sur la liste</Paragraphe>
                        <Button  type-button="border" @click="handleSubscribe(publication.id)"> 
            ...
            <Card class="houseWarming__lists--container">
                ...
                <div class="houseWarming__lists">
                    <div
                        v-for="(item, index) in publication.purchase.inscrits"
                        :key="index"
