<template>
  <Transition appear name="fade-step" mode="out-in"> 
    ...
    <MainCard :dataBreadcrumbs="breadcrumbData" >
      <Transition appear name="fade-step" mode="out-in">
        <div class="mb-4">
...
import InvoiceBalanceRow from "./InvoiceBalanceRow.vue";
import InvoiceBalanceTableHead from "./InvoiceBalanceTableHead.vue";
...
onMounted(async () => {
  store.dispatch('CollectiveGardenModule/getSelectedGardenInvoice')
})
...
watch(()=> store.getters['CollectiveGardenModule/getInVoices'],
  (value)=>{
    invoiceList.value = [
      {
            "id": 10,
