<template>
  <MainCard 
    ...
    :button="true" 
    @handleClickReturn="backTo"
  >
    <div class="council-report__container">
      <Title type="h3" label="Ajouter un rapport" class="council-report__title" />
      <hr class="__hr" />
      <div class="council-report__form-upload">
        <UploadImage 
          :key="`upload${key}`"
...
function handleChange(value: any) {
  if (value.hasOwnProperty("comment")) {
    ...
    fileList.value = value.map((element: any) => {
      return { file: element.originFileObj, category: "council" };
    });
  }
}
...
async function submitForm() {
  ...
  let success = await store.dispatch("SyndicModule/postCouncilReport", payload);
  if (success) {
